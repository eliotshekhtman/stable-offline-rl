...
The supported tasks are exactly `Reacher-v5`, `HalfCheetah-v5`, `Walker2d-v5`, `Lift`, and `Can`. Reacher, HalfCheetah, and Walker2d support generated, Minari, and clean-Minari data; Lift and Can support Robomimic data. Other tasks and incompatible task/source combinations fail during argument validation before creating artifacts. Walker2d uses the local SAC expert and the Minari datasets `mujoco/walker2d/simple-v0`, `mujoco/walker2d/medium-v0`, and `mujoco/walker2d/expert-v0`.

All datasets are split randomly by complete episode, without source stratification. Minari episodes, robomimic demonstrations, and generated rollouts share the same ordered transition schema: every `episode_id` occupies one contiguous block, and consecutive entries within that block are consecutive environment transitions. Generated `--num-samples` values are minimum transition counts before the train/test split. Each source independently collects complete trajectories until it meets its share of that minimum: `--composition 0.5 0.5 --num-samples 1000000` requests at least 500,000 clean and 500,000 noisy transitions, even if their episode lengths differ. The final trajectory can overshoot each source's quota; zero-share sources are skipped. Metadata records requested transition quotas plus realized trajectory and transition counts and fractions. Existing datasets and trained runs remain reusable when their recorded source counts already meet these quotas; undersized source components are not silently reused.
...
Use `--dataset-source clean-minari --dataset DATASET` to ablate between clean expert trajectories and one Minari quality dataset. `--minari-fraction` defaults to `0 0.25 0.5 0.75 1`; each fraction specifies the Minari share of the requested transitions, with the remaining transitions collected from the clean expert. Complete Minari episodes are selected from a seeded permutation without replacement until their quota is met. A request exceeding the available Minari transitions fails before collecting clean trajectories, instead of duplicating data. The 0% endpoint reuses an existing generated-clean dataset and model when its environment, expert, horizon, sample count, seed, split, and training settings match and its transition quota is met; the irrelevant historical clean-dataset `noise_scale` may differ.
